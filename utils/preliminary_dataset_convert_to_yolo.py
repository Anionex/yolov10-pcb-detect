"""
将PCB瑕疵初赛样例集转换为YOLO格式数据集
初赛数据已经是YOLO格式，只需要整合文件结构并统一类别映射
"""
...
# 配置
SRC_DIR = "./data/PCB_瑕疵初赛样例集"
OUTPUT_DIR = "./data/PCB_瑕疵初赛样例集/yolo_dataset"
...
# 初赛类别映射（初赛ID -> PKU统一ID）
# 初赛: 0-Mouse_bite, 1-Open_circuit, 2-Short, 3-Spur, 4-Spurious_copper
...
# 瑕疵类型目录（图像文件夹, 标注文件夹）
DEFECT_TYPES = [
    ("Mouse_bite_Img", "Mouse_bite_txt"),
    ("Open_circuit_Img", "Open_circuit_txt"),
    ("Short_Img", "Short_txt"),
    ("Spur_Img", "Spur_txt"),
    ("Spurious_copper_Img", "Spurious_copper_txt"),
]
...
def convert_dataset():
    ...
    # 处理每种瑕疵类型
    for img_dir_name, txt_dir_name in DEFECT_TYPES:
        img_dir = os.path.join(SRC_DIR, img_dir_name)
        txt_dir = os.path.join(SRC_DIR, txt_dir_name)
        
        if not os.path.exists(img_dir):
            print(f"⚠️  跳过 {img_dir_name} (目录不存在)")
            continue
        ...
        for img_file in os.listdir(img_dir):
            ...
            src_txt = os.path.join(txt_dir, txt_file)
